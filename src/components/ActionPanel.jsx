  ...
  return (
    <div style={{ flex: 1, padding: 10, overflow: "auto" }}>
      ...
      {game.cardDrawn && game.phase === "card" && (
        <div className="card-drawn" style={card({ marginBottom: 10, background: "linear-gradient(135deg, #1a1a0e, #12120e)", border: `1px solid ${S.gold}66`, boxShadow: `0 4px 20px rgba(201,168,76,0.15)` })}>
          <div style={{ fontSize: 14, lineHeight: 1.5, fontStyle: "italic" }}>{game.cardDrawn.text}</div>
      ...
      {game.phase === "buy" && isMyTurn && (
        <div style={{ display: "flex", flexDirection: "column", gap: 6, animation: "slideInUp 0.3s ease-out" }}>
          <button onClick={doBuy} style={{ ...btn({ fontSize: 14, padding: "10px" }), boxShadow: "0 4px 16px rgba(201,168,76,0.2)" }}>💰 {t.buy} ({cells[cp.position].price} {t.gold})</button>
      ...
      {game.phase === "auction" && game.auctionState && (
        <div style={card({ marginBottom: 10, animation: "fadeIn 0.3s ease-out" })}>
          ...
          <div style={{ fontSize: 12, marginTop: 4, display: "flex", justifyContent: "space-between" }}>
            <span>{t.currentBid}: <span style={{ color: S.gold, fontWeight: "bold" }}>{game.auctionState.highBid}</span> {t.gold}</span>
          </div>
          {game.auctionState.highBidder !== null && (
            <div style={{ fontSize: 12, color: S.textDim }}>{t.highBidder}: {game.players[game.auctionState.highBidder].name}</div>
          )}
      ...
      {game.phase === "endturn" && isMyTurn && (
        <div style={{ display: "flex", flexDirection: "column", gap: 6, animation: "fadeIn 0.3s ease-out" }}>
          ...
          <button onClick={endTurn} style={{ ...btn({ width: "100%", fontSize: 14, padding: "10px", marginTop: 4 }), boxShadow: "0 4px 16px rgba(201,168,76,0.2)" }}>→ {t.endTurn}</button>
        </div>
      )}

      {/* AI thinking */}
      {cp.isAI && game.phase !== "gameover" && (
        <div style={{ textAlign: "center", padding: 24, color: S.textDim, animation: "fadeIn 0.5s ease-out" }}>
          ...
          <div style={{ marginTop: 8, display: "flex", justifyContent: "center", gap: 4 }}>
            {[0,1,2].map(i => (
              <div key={i} style={{ width: 6, height: 6, borderRadius: "50%", background: S.gold, animation: `pulse 1s ease-in-out ${i * 0.3}s infinite`, opacity: 0.6 }} />
            ))}
          </div>
        </div>
      )}
