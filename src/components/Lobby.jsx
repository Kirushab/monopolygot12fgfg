...
export default function Lobby({ setScreen, lang, t, mp }) {
  ...
  if (mp.roomState && mp.roomState.state === "lobby") {
    return <RoomLobby mp={mp} lang={lang} t={t} l={l} setScreen={setScreen} onLeave={() => { mp.leaveRoom(); setView("browse"); }} />;
  }
    ...
    return (
      <div style={{ minHeight: "100vh", background: S.bg, color: S.text, fontFamily: S.font, padding: 20 }}>
        <div style={{ maxWidth: 600, margin: "0 auto" }}>
          ...
          <h2 style={{ color: S.gold }}>⚔️ {l("Онлайн", "Online")}</h2>
          ...
          <div style={card({ marginBottom: 12 })}>
            ...
            <input value={playerName} onChange={e => setPlayerName(e.target.value)} style={{ display: "block", width: "100%", background: S.bg2, color: S.text, border: `1px solid ${S.border}`, borderRadius: 6, padding: "8px 10px", fontSize: 14, fontFamily: S.font, marginTop: 4, boxSizing: "border-box" }} />
          </div>
          ...
          <div style={{ display: "flex", gap: 8, marginBottom: 16 }}>
            <input value={joinId} onChange={e => setJoinId(e.target.value.toUpperCase())} placeholder={l("Код комнаты...", "Room code...")} style={{ flex: 1, background: S.bg2, color: S.text, border: `1px solid ${S.border}`, borderRadius: 6, padding: "8px 10px", fontSize: 14, fontFamily: S.font, letterSpacing: 2 }} />
    ...
    return (
      <div style={{ minHeight: "100vh", background: S.bg, color: S.text, fontFamily: S.font, padding: 20 }}>
        <div style={{ maxWidth: 500, margin: "0 auto" }}>
          ...
          <h2 style={{ color: S.gold }}>⚔️ {l("Новая комната", "New Room")}</h2>

          <div style={card({ marginBottom: 12 })}>
            <label style={{ fontSize: 12, color: S.textDim }}>{l("Название", "Name")}</label>
            <input value={roomName} onChange={e => setRoomName(e.target.value)} placeholder={l("Игра престолов", "Game of Thrones")} style={{ display: "block", width: "100%", background: S.bg2, color: S.text, border: `1px solid ${S.border}`, borderRadius: 6, padding: "8px 10px", fontSize: 14, fontFamily: S.font, marginTop: 4, boxSizing: "border-box" }} />
          </div>
          ...
          <div style={card({ marginBottom: 12 })}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 6 }}>
              {[2, 3, 4].map(n => (
                <button key={n} onClick={() => setMaxPlayers(n)} style={{ background: maxPlayers === n ? S.gold : "transparent", color: maxPlayers === n ? S.bg : S.text, border: `1px solid ${S.gold}`, padding: "8px 20px", borderRadius: 6, cursor: "pointer", fontFamily: S.font }}>{n}</button>
              ))}
            </div>
          </div>
...
// ============================================================
function RoomLobby({ mp, lang, t, l, setScreen, onLeave }) {
    ...
    <div style={{ minHeight: "100vh", background: S.bg, color: S.text, fontFamily: S.font, padding: 20 }}>
      <div style={{ maxWidth: 550, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
          ...
          <div style={{ fontSize: 12, color: S.textDim, letterSpacing: 2, fontFamily: S.font }}>
            {l("КОД", "CODE")}: <span style={{ color: S.gold, fontWeight: "bold", fontSize: 16 }}>{room.id}</span>
          </div>
        </div>

        <h2 style={{ color: S.gold }}>⚔️ {room.name}</h2>
        ...
        {room.slots.map((slot, i) => (
          <div key={i} style={card({ display: "flex", alignItems: "center", gap: 10, marginBottom: 8, padding: "10px 14px" })}>
            <div style={{ width: 28, height: 28, borderRadius: "50%", background: slot.connected || slot.isAI ? S.gold + "33" : S.bg, border: `1px solid ${S.gold}44`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>
            ...
            {mp.isHost && slot.isAI && (
              <div style={{ display: "flex", gap: 4 }}>
                <select value={slot.aiPersonality || "random"} onChange={e => mp.updateSlot({ slotIndex: i, action: "set_personality", personality: e.target.value })} style={{ background: S.bg2, color: S.text, border: `1px solid ${S.border}`, borderRadius: 4, fontSize: 11, padding: "2px 4px", fontFamily: S.font }}>
            ...
            {slot.index === mp.mySlotIndex && !slot.isAI && i !== 0 && (
              <button onClick={() => mp.toggleReady()} style={btnOutline({ fontSize: 11, padding: "4px 12px", borderColor: slot.ready ? "#50c878" : S.gold, color: slot.ready ? "#50c878" : S.gold })}>
        ...
        {mp.isHost && (
          <div style={card({ marginTop: 12, marginBottom: 12 })}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
              {["classic", "quick"].map(m => (
                <button key={m} onClick={() => mp.updateConfig({ config: { mode: m } })} style={{ background: room.config.mode === m ? S.gold : "transparent", color: room.config.mode === m ? S.bg : S.text, border: `1px solid ${S.gold}`, padding: "6px 16px", borderRadius: 6, cursor: "pointer", fontFamily: S.font, fontSize: 12 }}>
            ...
            <div style={{ display: "flex", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
              {[1000, 1500, 2000].map(m => (
                <button key={m} onClick={() => mp.updateConfig({ config: { startMoney: m } })} style={{ background: room.config.startMoney === m ? S.gold : "transparent", color: room.config.startMoney === m ? S.bg : S.text, border: `1px solid ${S.gold}`, padding: "6px 14px", borderRadius: 6, cursor: "pointer", fontFamily: S.font, fontSize: 12 }}>
                  {m} {t.gold}
                </button>
              ))}
            </div>
